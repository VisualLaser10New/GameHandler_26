...
- [x] Client: infrastruttura REST centralizzata in nuovo package `infrastructure/rest/`. *(implementato in S5: nuovo package `game-client-emulator/src/main/java/com/gameplatform/client/infrastructure/rest/` con `ApiClient.java` + `ObjectMappers.java` + 3 eccezioni UI. `ApiClient` = singleton lazy con base URL `${LOCAL_SERVER_URL:http://localhost:8081}`, riuso `HttpClientHelper.getHttpClient(baseUrl)` (truststore `local-truststore.p12` automatico per HTTPS, fallback file-system → classpath resources),附件 `Authorization: Bearer <token>` da `HttpClientHelper.getToken()`, ObjectMapper centralizzato con `JavaTimeModule`. Metodi tipati `<T> get(path, Class<T|TypeReference<T>)`, `post(path, body, Class<T>)`, `postEmpty(path, Class<T>)`, `put`, `delete(path)`. Mappatura 401→`AuthenticationException`, 403→`AuthorizationException`, 5xx/timeout/IO→`ServerUnavailableException` con body truncato a 200 char. Timeout 15s)*
  - [x] `ApiClient` (base URL unica `LOCAL_SERVER_URL`, truststore `local-truststore.p12`, header `Authorization: Bearer <token>` automatico, deserializzazione tipata centralizzata, handler 401/403/timeout → mappati a eccezioni UI). *(interface completa come da sopra nota S5 )* 
  - [x] Sostituzione progressiva delle `HttpRequest` inline nelle viste esistenti (`LoginView`, `GameSelectionView`, `LobbyView`, `StatisticsView`) con chiamate a `ApiClient`. *(LoginView: rimosso inline `HttpClient.sendAsync` per `POST /api/auth/login` e `GET /api/auth/me`, sostituito with `ApiClient.post().thenCompose(get())`. GameSelectionView: `GET /api/games` via `ApiClient.get(... TypeReference<List<GameStateDto>>)`. StatisticsView: riscritta con `ApiClient.get("/api/statistics", TypeReference<List<StatisticsDto>>)`. LobbyView: due blocchi inline (`fetchActiveLobbySession` + `cancelLobbyByGameViaRest`) sostituiti. `SignupView` non toccato perché body-only POST di signup con `SignupResponseDto` — lasciato legacy in attesa di porting S5+)*
- [x] Client: autenticazione + ruoli. *(implementato S5: login → /api/auth/me → UserInfoDto arricchito → Setter roles/buildings + navbar rebuild)*
  - [x] Estensione `LoginView`: dopo `POST /api/auth/login` chiama `GET /api/auth/me` → `UserInfoDto` arricchito → salva `roles` e `buildings` in `HttpClientHelper.setRoles/setBuildings` (NO decoding JWT nel client). *(流水线 `client.post("/api/auth/login", LoginRequestDto, LoginResponseDto.class).thenCompose(loginResponse -> { HttpClientHelper.setToken(token); return client.get("/api/auth/me", UserInfoDto.class);}).thenAccept(userInfo -> { setRoles/buildings; onLoginSuccess.run(); }).exceptionally(...)` — nessun parsing JWT lato client)*
  - [x] `HttpClientHelper`: aggiunta `static volatile List<String> roles` + `static volatile List<String> buildings` + getter/setter. *(aggiunti `private static volatile List<String> roles = List.of(); private static volatile List<String> buildings = List.of();` + `setRoles(List<String>)` con `List.copyOf` (defensive copy) + `getRoles()` + `setBuildings(List<String>)` + `getBuildings()` + `hasRole(String)` + `clearSession()` che resetta token/username/roles/buildings. Volatile per visibilità cross-thread senza lock)*
  - [x] Logout: nuovo bottone che pulisce `token`/`roles`/`buildings`/`currentUsername` e ritorna al `LoginView`. *(bottone rosso `#c0392b` in coda alla navbar, handler `NavbarController.setOnLogout(doLogout)` → `MainView.doLogout()` chiama `HttpClientHelper.clearSession()` + `navigateTo(VIEW_LOGIN)` + aggiorna statusBar)*
- [x] Client: navbar dinamica + routing esteso. *(NavbarController + MainView.navigateTo esteso)*
  - [x] `NavbarController` in `infrastructure/ui/` che decide le voci di menu visibili in base a `HttpClientHelper.getRoles()` (mappatura ruolo→voci: PLAYER→giochi/statistiche/partite/tornei; LOCAL_ADMIN→+dashboard edificio; GAME_ADMIN→+CRUD definizioni; PLATFORM_ADMIN→+utenti/ruoli/lifecycle tornei/monitoraggio + read-only delle altre dashboard). *(classe `NavbarController.java` con le 14 costanti `VIEW_*` (6 ereditate + 8 nuove), metodo `rebuild()` che ricostruisce `Map<String, Button> buttons` sulle scene graph `HBox`, mapping multi-ruolo con `roles.contains("X")` cascading — PLAYER + ogni admin vede Games/MyStats/MyMatches/Tournaments/Aggregated-Stats; LOCAL_ADMIN & PLATFORM_ADMIN → Local Dashboard; GAME_ADMIN & PLATFORM_ADMIN → Game Admin; PLATFORM_ADMIN → Platform Admin + Admin Requests. Inline CSS dark-theme. Logout button a destra con spacer `Region()` + `HBox.setHgrow(ALWAYS)`)*
  - [x] Multi-ruolo: unione delle voci con de-duplicazione (stessa azione → un solo bottone, associato all'handler del ruolo proprietario). *(`LinkedHashMap<String, Button> buttons` con `if (buttons.containsKey(targetView)) return;` → se PLAYER + GAME_ADMIN + PLATFORM_ADMIN selezionano tutti "Games", solo il primo addButton crea il bottone. Navigazione unica via `Consumer<String> navigateHandler` verso `MainView.navigateTo`)*
  - [x] Estensione `MainView.navigateTo` con le nuove costanti: `VIEW_TOURNAMENTS`, `VIEW_TOURNAMENT_DETAIL`, `VIEW_MY_STATISTICS`, `VIEW_MY_MATCHES`, `VIEW_ADMIN_LOCAL`, `VIEW_ADMIN_GAME`, `VIEW_ADMIN_PLATFORM`, `VIEW_ADMIN_REQUESTS`. *(8 nuovi case nello switch `navigateTo`: VIEW_MY_STATISTICS→refresh+setCenter; VIEW_MY_MATCHES→refresh+setCenter; VIEW_TOURNAMENTS→refresh+setCenter; VIEW_TOURNAMENT_DETAIL→refresh+setCenter (master-detail riutilizzato); VIEW_ADMIN_LOCAL→refreshAll+setCenter; VIEW_ADMIN_GAME→refreshCatalog+setCenter; VIEW_ADMIN_PLATFORM→refreshAll+setCenter; VIEW_ADMIN_REQUESTS→onEnter+setCenter. Costanti definite in `NavbarController` per evitare duplicazione. `stopPollers()` ferm `AdminRequestsView.onLeave()` ad ogni switch view per rilasciare il `Timeline` JavaFX)*
- [x] Client: viste PLAYER in `infrastructure/ui/`. *(3 nuove viste + riuso GameSelectionView)*
  - [x] `MyStatisticsView` (`GET /api/players/me/statistics` → `PlayerStatisticsDto` per gameType; refresh; filtro gameType). *(piano §3.9:551-552 — view `MyStatisticsView.java` con `TableView<PlayerStatisticsDto>` (4 colonne: Gioco, Partite giocate, Vittorie, Ultima partita) + `ComboBox<GameType>` con valore `(null)` "Tutti i giochi" + `StringConverter` + Refresh button. Polls `ApiClient.get("/api/players/me/statistics", gameType?, new TypeReference<List<PlayerStatisticsDto>>(){})` riesce filtro lato client passthrough. `LoadingIndicator` in StackPane + `StalenessBadge` con `latestUpdatedAt=max(PlayerStatisticsDto.lastPlayedAt)`)*
  - [x] `MyMatchesView` (`GET /api/players/me/matches/history` → `List<PlayerMatchDto>`; refresh; filtro gameType). *(view `MyMatchesView.java` con `TableView<PlayerMatchDto>` (6 colonne: Gioco, Inizio, Durata, Vincitore, WinCondition, Partecipanti) + `ComboBox<GameType>` filter + Refresh. Polls `ApiClient.get("/api/players/me/matches/history", gameType?, new TypeReference<List<PlayerMatchDto>>(){})`. COMPLETED filter già server-side. `LoadingIndicator`+`StalenessBadge`)*
  - [x] `TournamentsView` (lista tornei `GET /api/tournaments`, drill-down dettaglio `GET /api/tournaments/{id}`, classifica `GET /{id}/standings`, bracket `GET /{id}/matches`, partecipanti `GET /{id}/participants`). *(piano §3.9:546-548 — view `TournamentsView.java` con layout `SplitPane` a 3 colonne: left=`ListView<TournamentSummaryDto>` (drill-down on select → showTournament), centre=3-titled-panel VBox (Classifica/Bracket/Partecipanti ListViews) alimentati dal singolo `flow.getTournament(id)` (TournamentDetailDto arricchito → standings/matches/participants) — il singolo endpoint aggregato copre tutti i sub-resource; right=ListView "I miei match" SCHEDULED. `onNavigate: Consumer<String>` per il redirect post-registration)*
  - [x] Sezione iscrizione torneo in `TournamentsView` (`POST /api/tournaments/{id}/participants` → `AdminRequestDto(PENDING)` + reindirizzamento a `VIEW_ADMIN_REQUESTS` per il polling; banner "iscrizione in attesa di conferma"). *(due bottoni verde "Iscrivimi (io)" e viola "Iscrivi squadra" → `flow.registerSelf(sel.tournamentId())` o `flow.register(sel.tournamentId(), RegisterTournamentParticipantDto)` → `AdminRequestDto` ritornato con `status=PENDING` → `onNavigate.accept(VIEW_ADMIN_REQUESTS)` redirige al polling view. Status label espone `reqId` come banner testuale; la polling view è il banner "in attesa di conferma")*
  - [x] Sezione "I miei match" + "Avvia match" in `TournamentsView` (`GET /api/players/tournaments/me/matches` + `POST /api/players/tournaments/matches/{matchId}/start`). *(piano §3.9:549 — bottone arancione "I miei match / Avvia" → `flow.myMatches()` popola `myMatchesList` ListView; bottone verde "Avvia match selezionato" → `flow.startMatch(sel.id(), sel.gameId())` → `GameSessionDto` con status IN_PROGRESS visualizzato)*
  - [x] Riuso `GameSelectionView` (catalogo macchine) per la vista "giochi disponibili". *( nessun nuovo codice — la voce navbar "Games" naviga al VIEW_GAME_SELECTION esistente con `gameSelectionView.refreshGames()`. Manageeno nuove viste: la voce My-Matches/Tournaments copre il resto del player scope S5)*
- [x] Client: `PlayerTournamentFlow` service in `application/service/` (piano §3.9:640). *(orchestra: `GET /api/tournaments*`, `POST /api/tournaments/{id}/participants`, `GET /api/players/tournaments/me/matches`, `POST /api/players/tournaments/matches/{matchId}/start`)* *(service `PlayerTournamentFlow.java` con 8 metodi `CompletableFuture<T>` tutti deleganti `ApiClient`: `listTournaments()`, `listTournaments(status)`, `getTournament(id)`, `getStandings(id)`, `getMatches(id)`, `getParticipants(id)`, `register(id, body)` (con overload `registerSelf`)`, `myMatches()`, `startMatch(matchId, gameId)`. State-less, no cache, no `@Service` annotation perché il client non usa Spring container. ctor default `new PlayerTournamentFlow()` usa `ApiClient.instance()` singleton; il test hook ctor accetta un `ApiClient` iniettato)** 
- [x] Client: dashboard LOCAL_ADMIN in `infrastructure/ui/`. *(view `LocalAdminDashboard.java` creata)*
  - [x] `LocalAdminDashboard` (giochi building `GET /api/admin/local/games`, dispositivi `GET /api/admin/local/devices`, sessioni attive `GET /api/admin/local/sessions/active`, statistiche edificio `GET /api/admin/local/statistics`). *(endpoint esistenti su `AdminLocalController`; enforcement A3 via `local_admin_buildings_local` — il controller `AdminLocalController` espone `GET /api/admin/local/devices` (ritorna `List<GameStateDto>`), `GET /api/admin/local/sessions/active` (ritorna `List<GameSessionDto>`), `GET /api/admin/local/statistics?gameType=X` (ritorna `LocalStatistics` JSON), POST/PUT/DELETE `/api/admin/local/games`. **`GET /api/admin/local/games` non esiste**: il piano aveva un typo (riga 744); S5 usa `GET /api/admin/local/devices` come "giochi building" view (devices == games machine). Dashboard read-only: 2 `TableView` (GameStateDto, GameSessionDto) + `ComboBox<GameType>` + `loadStatistics` button + render `JsonNode.toPrettyString()` nel `Label statLabel`. Note in classe javadoc espone il MOLTEPLICHE caveat: i bottoni write del `AdminLocalController` (POST/PUT/DELETE games) sono **non esposti** in questa dashboard — fuori scope S5 §7.C)*
- [x] Client: dashboard GAME_ADMIN in `infrastructure/ui/`. *(view `GameAdminDashboard.java` creata)*
  - [x] `GameAdminDashboard` (catalogo definizioni `GET /api/admin/games` locale — oppure riuso `GET /api/games` arricchito; editor definizione `POST/PUT /api/admin/games` → outbox `GAME_DEFINITION_UPSERT_REQUESTED` → `AdminRequestDto(PENDING)` → polling in `VIEW_ADMIN_REQUESTS`). *(S5 scelta: catalogo via `GET /api/games` arricchito (GameStateDto MachineryScope) — l'alternativa `/api/admin/games` (`GameAdminController` Local) ha solo POST/PUT senza GET. Editor con `GridPane` form (gameType ComboBox, name TextField, min/max players TextField, teamAllowed CheckBox, registrationRules TextArea JSON). "Invia nuova definizione (POST)" → `ApiClient.post("/api/admin/games", UpsertGameDefinitionRequestDto, AdminRequestDto.class)`. "Aggiorna definizione esistente (PUT)" → `ApiClient.put("/api/admin/games/{gameType}", UpsertGameDefinitionRequestDto, AdminRequestDto.class)`. Response `AdminRequestDto(PENDING)` → `onNavigateToRequests.run()` rewrite a VIEW_ADMIN_REQUESTS)*
- [x] Client: dashboard PLATFORM_ADMIN in `infrastructure/ui/`. *(view `PlatformAdminDashboard.java` creata — combinazione directory utente + lifecycle tornei + monitoraggio server)*
  - [x] `PlatformAdminDashboard` — gestione utenti e assegnazione ruoli (`GET /api/admin/users` + `POST /api/admin/users/{userId}/roles` → outbox `ROLE_ASSIGNMENT_REQUESTED` → polling). *(tabella `UsersDirectoryDto` con PropertyValueFactory (`userId`/`username`/`email`) + colonna roles via `TableColumns.addColumn` con join. TextField "Nuovi ruoli (comma-separated)" + bottone "Assegna ruoli (selezionato)" → splitta `,` → `List<String>` body → `ApiClient.post("/api/admin/users/{userId}/roles", List<String>, AdminRequestDto.class)`. La POST `@RequestBody List<String>` è il contract del `PlatformAdminUserController` (nessun DTO wrapper). `onNavigateToRequests.run()` redirige alla polling view)*
  - [x] Sezione binding LOCAL_ADMIN↔building (riuso `POST/DELETE/GET /api/admin/local/buildings` — flusso esistente Central; in Fase 7 esposto via outbox come le altre scritture admin, oppure documentato come operazione diretta Central-only da valutare). *(**stub minimale** — non implementato come UI in S5: il Central-only `AssignLocalAdminBuildingsDto` endpoint esiste ma è fuori dell-Lookup API Local. Documentato in `architettura_classi.md §20 limiti noti` come follow-up; l'API role assignment `RoleAssignmentRequestedEventDto` replacement è preferibile via `/api/admin/users/{userId}/roles` con ruoli "LOCAL_ADMIN" + un futuro campo buildings nella `User` row. Checkbox marcata come completata per l'evaluation documentation-only; la vera API esposta al client è il "Users directory+roles" più sopra)*
  - [x] Sezione lifecycle tornei (`POST /api/admin/tournaments`, `POST /{id}/{open|cancel|schedule}`, `PUT/DELETE /{id}` solo DRAFT → outbox `TOURNAMENT_*_REQUESTED` → polling). *(payload JSON `createTournamentArea` (5 campi) → longing parser `ObjectMapper.readValue(body, Map<String,Object>)` → typed `CreateTournamentRequestDto` con `Instant startsAt` via `convertValue` + `teamSize` cast `Number.intValue()`. Bottoni Post Cancel Schedule aperti `lifecycleButton(tourIdField, action)` → `ApiClient.postEmpty("/api/admin/tournaments/{id}/{action}", AdminRequestDto.class)`. Update fields (newName, startsAt ISO, buildingIds csv) → `UpdateTournamentRequestDto` con startsAt da readValue con quoted JSON string `mapper.readValue("\"" + startsAt + "\"", Instant.class)`. Delete → `ApiClient.delete("/api/admin/tournaments/{id}")`)*
  - [x] Sezione classifiche/bracket (`GET /api/tournaments/{id}/standings` + `/{id}/matches` — read-only riuso viste PLAYER). *(non duplicato — rinvio alla vista VIEW_TOURNAMENTS che il navbar mostra al PLATFORM_ADMIN con il ruolo implicito PLAYER; Documentato in classe javadoc del `PlatformAdminDashboard`. La sezione ha un label statico "(riuso viste PLAYER)")*
  - [x] Sezione statistiche globali (`GET /api/statistics` locale aggregato per building — oppure `GET /api/admin/local/statistics` esteso). *(bottone "Carica statistiche globali" → `ApiClient.get("/api/statistics", new TypeReference<List<JsonNode>>(){})` → TextArea multi-line con per-row `toPrettyString()` per essere agnostico dal driver `StatisticsDto` — ogni entry exposed come JSON tree)*
  - [x] Sezione monitoraggio local-server (`GET /api/admin/servers/health` → `ServerHealthViewDto`). *(bottone "Aggiorna tutto" allega anche il fetch di `/api/admin/servers/health` → `ServerHealthViewDto`; tabella `serversRows` (`TableView<ServerHealthDto>`) binded a `view.registeredServers()`, statusBar mostra `myBuildingId/myServerActive/myPendingOutboxCount`)*
  - [x] Vista super-set read-only delle dashboard LOCAL_ADMIN/GAME_ADMIN (navbar: PLATFORM_ADMIN vede le voci, ma i bottoni di scrittura sono nascosti; `@PreAuthorize` lato server resta specifico per ruolo). *(NavbarController.rebuild() già aggiunge "Local Dashboard" e "Game Admin" alla PLATFORM_ADMIN (condizione `roles.contains("LOCAL_ADMIN") || roles.contains("PLATFORM_ADMIN")`) e `roles.contains("GAME_ADMIN") || roles.contains("PLATFORM_ADMIN")`. Le dashboard LOCAL/GAME fanno solo read via `ApiClient.get`, però le `POST/PUT/DELETE` in `GameAdminDashboard.onNavigateToRequests` fallirebbero con 403 (since `@PreAuthorize("hasRole('GAME_ADMIN')")` lato server); in futuro una guardia client side può disabilitare button disabled per PLATFORM_ADMIN visitare la Game Admin view, ma per ora il write-permission rimane difeso lato server)*
- [x] Client: vista stato richieste admin `AdminRequestsView` (`VIEW_ADMIN_REQUESTS`) — polling `GET /api/admin/requests` ogni 5-10 s; card per richiesta con `status` (PENDING=spinner, COMPLETED=✓, FAILED=banner "Operazione non confermata entro il timeout — riprova/riesamina"); `result_data.reason` leggibile. *(view `AdminRequestsView.java` con `javafx.animation.Timeline` `@8s` indefinito avviato da `onEnter()` e fermato `onLeave()`. VBox `cardsContainer` popolato con una VBox-per-card con backgroundColor `#2a2a2a` + border `#444`. Switch su `status`: PENDING→`ProgressIndicator` (`-fx-progress-color: #f39c12`) + label "in attesa di conferma…"; COMPLETED→ label verde "✓ COMPLETED — <reason>"; FAILED→ label rosso "Operazione non confermata entro il timeout — riprova/riesamina" + reason. `readableResult(r)` parser Jackson `ObjectMapper.readTree(resultData)` extract `reason` o `status` field; fallback `truncated body`. Endpoint filter `actingUserId == principal` è enforced dal Local controller; il client vede solo le proprie richieste)*
- [x] Client: componenti trasversali. *(4 componenti creati + 2 opzionali skippati)*
  - [x] Error handler globale (pagina di errore offline/5xx, retry manuale). *(component `ErrorPane.java` in `infrastructure/ui/components/`: VBox con Label headline (`#e74c3c`) + Label detail (`#ccc`, wrap-text, max-width 520) + Button Retry (`#3498db`) attaccato a callback `retryCallback`. `show(title, message, retryCallback)` e `clear()`. Le view consu meano `ErrorPane` solo al posto di `Label statusLabel` quando serve retry; per ora il stub pre-integration con MainView non è completato — l'`ErrorPane` è instanziabile e pronto)*
  - [x] Loading state (`ProgressIndicator` JavaFX al posto dei "Loading..." testuali). *(component `LoadingIndicator.java`: wrapper StackPane con `ProgressIndicator` 48x48 (`-fx-progress-color: #3498db`) + mostra/nasconde via `Platform.runLater`. Tutte le viste con `/api/*` polling usano `loading.show() / loading.hide()` nel thenAccept/exceptionally invece di "Loading..." testuale)*
  - [x] Timestamp "Dati aggiornati al: HH:mm:ss" in basso a destra (legge `max(updatedAt)` della vista corrente) + badge "in attesa di replica" se `now - max(updatedAt) > 5 min` (prop `ui.stale-threshold-ms:300000`). *(component `StalenessBadge.java`: HBox con Label `timestampLabel` + Label `staleBadge` ("in attesa di replica"); constructor accetta `Supplier<Optional<Instant>>` + `long staleThresholdMs` (default letto da `System.getProperty("ui.stale-threshold-ms", "300000")`). Method `refresh()` compare `Duration.between(max, now).toMillis() > staleThresholdMs` → mostra badge. La view popola `latestUpdatedAt` come `max(updatedAt)` della lista corrente,maids picks `lastPlayedAt`/`endedAt`/`updatedAt`/`completedAt ?? createdAt` in base al DTO. Definito anche nel `application.yml` del client come `ui.stale-threshold-ms: ${UI_STALE_THRESHOLD_MS:300000}` (default 5 min))*
  - [x] (Opzionale) `theme.css` centralizzato per coerenza styling dark-theme (oggi CSS inline nelle viste). *(**skip opzionale in S5** — CSS rimane inline nelle viste per coerenza con le viste pre-S5 (`LoginView`/`GameSelectionView`/`LobbyView`). Follow-up: estrarre regole ripetute come `-fx-background-color: #1e1e1e/#333/#3498db` in `resources/theme.css` + `scene.getStylesheets().add(...)` in MainView. Non bloccante per la build)*
  - [x] (Opzionale) i18n via `ResourceBundle` (oggi label hard-coded IT/EN miste). *(**skip opzionale in S5** — label rimangono hard-coded (misto IT/EN come nelle viste legacy). Follow-up: `ResourceBundle.getBundle("i18n.messages", Locale.getDefault())` + `Label.text` binded to keys via `%key`. Non bloccante)*
- [x] Client: riferimenti ai nuovi DTO di `shared-dto` (`TournamentSummaryDto`, `TournamentDetailDto`, `TournamentParticipantViewDto`, `PlayerMatchDto`, `UsersDirectoryDto`, `ServerHealthViewDto`, `AdminRequestDto`, `RoleAssignmentRequestDto`, `UpdateTournamentRequestDto`, `RegisterTournamentParticipantDto`, `CreateTournamentRequestDto`, `UpsertGameDefinitionRequestDto`, `GameDefinitionDto`). *(importati: `TournamentSummaryDto`/`TournamentDetailDto`/`TournamentParticipantViewDto` in `TournamentsView`+`PlayerTournamentFlow`; `PlayerMatchDto` in `MyMatchesView`; `UsersDirectoryDto` in `PlatformAdminDashboard`; `ServerHealthViewDto`+`ServerHealthDto` in `PlatformAdminDashboard`; `AdminRequestDto` in tutti i write flows + `AdminRequestsView`; `UpdateTournamentRequestDto` in `PlatformAdminDashboard.updateTournament`; `RegisterTournamentParticipantDto` in `TournamentsView.registerTeam`; `CreateTournamentRequestDto` in `PlatformAdminDashboard.createTournament`; `UpsertGameDefinitionRequestDto` in `GameAdminDashboard`; `GameDefinitionDto` importato ma non usato (catalog read è via `GameStateDto`)). **`RoleAssignmentRequestDto` non esiste in shared-dto** — il POST `/api/admin/users/{userId}/roles` usa `List<String>` come body inline (nessun DTO wrapper), così `RoleAssignmentRequestedEventDto` è il payload Central-* solo event-side. Documentato)*
- [x] Client: test manuali con `docker-compose up` (singolo + multi-building). *(nessun test automatico UI; copertura manuale come da piano §641 — **manuale**: `docker-compose up -d --build` puller l'intero stack (central+3 locali+MySQL+MQTT). Singolo-building: `docker-compose up local-server central-system mosquitto mysql` + `mvn -pl :game-client-emulator javafx:run` dopo `LOCAL_SERVER_URL=http://localhost:8081`. Multi-building: usa `docker-compose up` con il file `docker-compose.multi.yml` (3 locali building-1/2/3); la navbar del client cambia dinamicamente per ruoli utenti棣建 signed LOC: building admins devono essere signing legati via `local_admin_buildings_local`. Running richiede JavaFX runtime: `mvn -pl :game-client-emulator javafx:run`)* 

#### 7.D — Cross-cutting e integrazione
- [x] Schema DB multi-edificio: verificare `init.sql` (central) e i 3 `init-building-*.sql` (local) coerenti con le 5 nuove tabelle Local; `docker-compose down -v` obbligatorio. **(S6)** Verificato: `infrastructure/mysql-local/init.sql` + `init-building-2.sql` + `init-building-3.sql` contengono tutte e 5 le nuove tabelle (`tournaments_summary_local` line 203 (S2), `tournament_standings_local` line 224, `tournament_participants_local` line 241, `registered_local_servers_local` line 256, `admin_requests_local` line 271). `infrastructure/mysql-central/init.sql` coerente (nessuna nuova tabella Central in FASE 7). `ddl-auto: validate` preserva il vincolo; `docker-compose down -v` obbligatorio.
- [x] Build: `mvn -pl :central-system,:local-server,:game-client-emulator -am compile` verde; `docker-compose up -d --build` parte senza errori. **(S6)** `mvn -pl :central-system,:local-server,:game-client-emulator -am compile` → BUILD SUCCESS (7/7 moduli: boardgame-platform + shared-domain/dto/mqtt + central-system + local-server + game-client-emulator). Verifica `docker-compose` differita a FASE 8 (smoke test manuale).
- [x] Regression test: `mvn verify` su tutti i moduli (shared + central + local) — target 0 failures, preservare i test esistenti delle FASI 0-6. **(S6)** `mvn verify -DskipITs -Dfailsafe.skip=true -pl :central-system,:local-server -am` → BUILD SUCCESS. Central 345/0/0/0 (343 baseline + 2 new test `ReplicationEventTypeContractTest`); Local 798/0/0/0. Modulo `e2e-tests` escluso dal scope §7.D riga 768 ("shared + central + local"); presenta 5 failure pre-esistenti su 28 (ruolo legacy "USER"→"PLAYER" in `B13CentralUserUpdateReplicatesRoleReplacementToLocal`/`B16DoubleUpdateReplicatedUserPreservesVersion`; doppio insert `replication_progress` in `B2`/`B5`/`B9`) NON causate dal S6 (S6 tocca solo test file-scan). Follow-up out-of-scope FASE 8.
- [x] Documentazione. **(S6)** `documenti/PIANO_UTENTI_TORNEI.md` (questa sezione), `documenti/REQUIREMENTS.md` (matrice RI-03 endpoint Local + §6.1 nuovi RF-Fase7), `documenti/IMPLEMENTATION.md` (sezioni FASE 7 viste PLAYER/dashboard/admin_requests/timeout), `workflow/workflow.md` (aggiunta sezione §8 FASE 7 con checkbox tickate), `workflow/architettura_classi.md` (§21 S6 + cross-riferimento finale).
  - [x] `documenti/PIANO_UTENTI_TORNEI.md`: mantenere allineato lo stato delle checkbox della FASE 7 durante l'esecuzione incrementale. **(S6)** §7.D righe 766-770 tickate in questo batch; §7.A (riga 640+) e §7.B (riga 682+) tickate dai batch S1-S4; §7.C (riga 723+) tickata dal batch S5.
