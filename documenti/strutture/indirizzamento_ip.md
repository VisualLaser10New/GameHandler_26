...
*   **Porta del server (SSL abilitato):** `8080` (definito alla riga 14 come `server.port: 8080`).
...
*   **Origini CORS consentite (Frontend/Console):** `http://localhost:3000` (riga 66). Può essere sovrascritto tramite `CORS_ALLOWED_ORIGINS`.
...
*   **Porta del server (SSL abilitato):** `8081` (definito alla riga 16 come `server.port: ${PORT:8081}`).
*   **URL del Central System:** `https://localhost:8080` (riga 27 sotto `app.central-system-url`). Sovrascritto dalla variabile `CENTRAL_SYSTEM_URL`.
*   **URL del Broker MQTT locale:** `tcp://localhost:1883` (riga 43 sotto `mqtt.broker-url`). Sovrascritto da `MQTT_BROKER_URL`.
*   **Base URL del Local Server stesso (comunicato al Central per callback):** `https://local-server-1:8081` (riga 26 sotto `app.local-base-url`). Sovrascritto da `LOCAL_BASE_URL`.
...
*   **URL del Local Server:** `http://localhost:8081` (riga 9 sotto `app.local-server-url`). Sovrascritto dalla variabile `LOCAL_SERVER_URL`.
*   **URL del Broker MQTT:** `tcp://localhost:1883` (riga 12 sotto `mqtt.broker-url`). Sovrascritto da `MQTT_BROKER_URL`.

*(Nota: Nel codice UI dell'emulatore, ad esempio in [LoginView.java](file:///c:/Users/VLT14/Documents/UNI/PISSIR/Progetto/gamehandler-platform/game-client-emulator/src/main/java/com/gameplatform/client/infrastructure/ui/LoginView.java#L128), viene utilizzato come fallback `https://localhost:8081` qualora la variabile `LOCAL_SERVER_URL` non sia definita).*
...
*   [docker-compose.yml](file:///c:/Users/VLT14/Documents/UNI/PISSIR/Progetto/gamehandler-platform/docker-compose.yml):
    ...
    *   **Local Server 1 -> Database:** `jdbc:mysql://local-db-1:3306/local_db` (riga 106).
    *   **Local Server 1 -> Broker MQTT:** `ssl://mqtt-broker-1:8883` (riga 107).
    *   **Local Server 1 -> Central System:** `https://central-system:8080` (riga 110).
    *   **Game Client 1 & 2 -> Local Server 1:** `https://local-server-1:8081` (righe 148, 166).
    *   **Game Client 1 & 2 -> Broker MQTT:** `ssl://mqtt-broker-1:8883` (righe 147, 165).
