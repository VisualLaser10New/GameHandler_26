...
| 5 | `shared/shared-dto/src/main/java/com/gameplatform/shared/dto/TournamentSummaryEventDto.java` | NUOVO | shared-dto | Record 16 campi (eventId, eventType, tournamentId, name, gameType, teamBased, teamSize, status, startsAt, endsAt, buildingIds, participantsCount, updatedAt, deleted, originatingRequestId, errorMessage) + **2 compact ctor** retrocompatibili: 15-arg (senza `errorMessage`, default `null`) e 14-arg (senza `deleted`/`originatingRequestId`/`errorMessage`, default `false`/`null`/`null`, pattern `UserSyncDto.java:14-16`). `errorMessage` è non-null quando il Central rifiuta una `*_REQUESTED` (es. transizione di stato illecita o torneo non trovato) — il Local `TournamentSummarySyncService` chiude la riga `admin_requests_local` come `FAILED` invece di `COMPLETED` (fix BUG-CANCEL-PENDING, vedi §16.2/§19.3) |
| 6 | `central-system/src/main/java/com/gameplatform/central/application/service/TournamentService.java` | ESTESO | application/service | `implements UpdateTournamentUseCase, DeleteTournamentUseCase, EmitTournamentSummaryUseCase`; ctor 7-arg `@Autowired` (tournamentRepository, tournamentBuildingRepository, tournamentParticipantRepository, gameDefinitionRepository, clock, outboxEventRepository, objectMapper) + legacy 5-arg delegante (senza outbox deps); metodi `update`/`delete`/`emitSummary` `@Transactional`; helper `writeOutboxEvent` inline 6-arg (incl. `errorMessage`) null-safe; `writeRejectionEvent` per i path `*_REQUESTED` rifiutati |
...
- **Nuove porte in** (`central-system/.../domain/ports/in/`):
  ...
  - `EmitTournamentSummaryUseCase.emitSummary(TournamentId tournamentId, String originatingRequestId)` + default `emitSummary(TournamentId)` (originatingRequestId=null): emette un `TOURNAMENT_SUMMARY_UPSERTED` di ack per i path `*_REQUESTED` che non producono naturalmente uno upsert di sommario (es. `TOURNAMENT_SCHEDULE_REQUESTED`, i cui `TOURNAMENT_MATCH_SCHEDULED` non portano `originatingRequestId`); il Local `TournamentSummarySyncService.markCompletedIfRequested` lo tratta come segnale di completamento. No-op (WARN log) se le deps outbox sono `null` (ctor legacy 5-arg).
...
- **`errorMessage` come marker di FAILED** (fix BUG-CANCEL-PENDING, attivato sul path `*_REQUESTED`): quando il Central gestisce una `*_REQUESTED` e la rifiuta (es. `InvalidTournamentStateException` su `open`/`cancel`/`schedule`/`update`/`delete`, o `TournamentNotFoundException`), il `TournamentService.writeRejectionEvent` emette un `TOURNAMENT_SUMMARY_UPSERTED` con `deleted=false` + `errorMessage` = reason leggibile. Il Local `TournamentSummarySyncService.markCompletedIfRequested` rileva `errorMessage != null` e chiama `AdminRequestRepository.markFailed(requestId, resultData, now)` invece di `markCompleted` — chiudendo il loop immediatamente (anziché attendere il timeout 60s di `AdminRequestTimeoutService` che superficerebbe come vaga "TIMEOUT").
...
- Outbox: il `AdminRequestOutboxWriter` scrive `OutboxEvent(id=requestId, eventType="*_REQUESTED", payload=serializedDto, status="PENDING", createdAt=now)` con lo stesso pattern del S2 (codec JSON via `ObjectMapper` injected). Il `SyncEventProcessor` del Central (S3 — 9 branch `*_REQUESTED`) consuma questi eventi, applica la mutazione e ritorna il DTO di ritorno (`TournamentSummaryEventDto`, `UserSyncDto`, `GameDefinitionEventDto`, `TournamentStandingsEventDto`, `TournamentParticipantsEventDto`, `LocalServerRegistryEventDto`) con `originatingRequestId=requestId`. Il `*SyncService` locale chiama `AdminRequestRepository.markCompleted(requestId, resultData, now)` — condizionale `WHERE status='PENDING'` quindi idempotente su re-delivery. Nel caso del `TournamentSummarySyncService`, se il `TournamentSummaryEventDto` di ritorno porta `errorMessage != null` (Central ha rifiutato la `*_REQUESTED`, es. transizione illecita o torneo non trovato), il sync service chiama invece `AdminRequestRepository.markFailed(requestId, resultData, now)` con `result_data={"reason":<errorMessage>}` — fix BUG-CANCEL-PENDING che evita il vagabondaggio in "TIMEOUT" sul client.
...
- `LoginView` API pubblica `setOnLoginSuccess(Runnable)` / `setOnNavigateToSignup(Runnable)` / `reset()` invariata. `performLogin()` signature pubblica rimasta; il body è passato da `HttpClient.sendAsync` + `ObjectMapper.readValue` in line a `ApiClient.post().thenCompose(get())` chain.
...
- **`GET /api/admin/local/games` spec bug** (riga 744 piano): il controller reale esporta solo `/devices` (+ POST/PUT/DELETE `/games`). S5 usa `/devices` come "vista giochi del building", documentato nel `LocalAdminDashboard` javadoc — write-through CRUD/non-read admin Local_S5_skip.
...
- **`TournamentDetailDto` = single endpoint aggregato ma `TournamentsView` rispetta i `standalone` `standings|matches|participants` sub-endpoint**: il client chiama principalmente `flow.getTournament(id)` (1 endpoint che ritorna tutte le 3 liste aggregate). Le chiamate isolate a `flow.getStandings(id)`/`flow.getMatches(id)`/`flow.getParticipants(id)` sono disponibili nel `PlayerTournamentFlow` ma non usate dalle viste S5 (ridondanti). Per la coerenza con la spec §7.C riga 738 "classifica `GET /{id}/standings`, bracket `GET /{id}/matches`, partecipanti `GET /{id}/participants`" — le view S5 li riceve dal dettaglio aggregato ma la specificazione delle risorse è ben nota e navigabile dal flow service `PlayerTournamentFlow`.
...
- **FASE 8 — `GET /api/admin/local/games` alignment**: allineare il piano riga 744 (bug typo) o aggiungere il `@GetMapping("/games")` endpoint al `AdminLocalController` (returns `List<GameStateDto>` games del building) per essere esplicitamente distinto da `/devices` nel caso in cui in futuro la superficie venga ampliata (es. catalogo "logico" di definizioni locali allowed nel building ≠ dispositivi fisici).
...
1. **Chiusura gap S3-A `EventTypeContractTest`**: la veste S3-A del `SyncEventProcessor` aveva aggiunto 9 branch `*_REQUESTED` (`ROLE_ASSIGNMENT_REQUESTED` + `GAME_DEFINITION_UPSERT_REQUESTED` + 7 `TOURNAMENT_*_REQUESTED`/`PARTICIPANT_REGISTER_REQUESTED`) con un `// TODO ONDATA-2` che ne differiva l'aggiunta al set `EXPECTED_EVENT_TYPES` difeso dal test, perché i producers Local non erano ancora emessi nel batch S3 Central. S4 ha emesso i 9 literal Local-side (W6/W9/W10/W12 use case); S6 chiude il gap aggiungendoli al set (15 totali: 6 baseline + 9 FASE 7) e rimuovendo il TODO. Verifica strutturale bidirezionale preservata (ogni literal è gestito Central-side ed emesso Local-side).
