...
/**
 * Slice test for {@link PlatformAdminUserController} covering
 * {@code POST /api/admin/users/{userId}/roles} happy + unauthorized paths.
 ...
 * literal ({@code "building-1"}).</p>
 *
 * <p>Coverage:
 * <ul>
 *   <li>{@code POST /api/admin/users/{userId}/roles} 202 on a valid JSON array of role strings;</li>
 *   <li>{@code POST .../roles} 401 when no authenticated user resolves (currentUserId empty).</li>
 * </ul>
 */
@ExtendWith(MockitoExtension.class)
class PlatformAdminUserControllerTest {

    private static final String BUILDING_ID = "building-1";

    private final ObjectMapper objectMapper = new ObjectMapper().registerModule(new JavaTimeModule());

    @Mock AssignRoleRequestedUseCase assignUseCase;
    ...
    void setup() {
        mvc = MockMvcBuilders.standaloneSetup(
                        new PlatformAdminUserController(assignUseCase, currentUserService, BUILDING_ID))
    ...
    private static AdminRequestDto adminRequestDto() {
        return new AdminRequestDto(
                "req-1", "ROLE_ASSIGNMENT_REQUESTED", "u-1", "PLATFORM_ADMIN", BUILDING_ID,
                "{}", "PENDING", null, Instant.parse("2026-07-12T10:00:00Z"), null, "req-1");
    }

    @Test
    void assignRoles_202_onValidRolesArrayBody() throws Exception {
        when(currentUserService.getCurrentUserId()).thenReturn(Optional.of(new UserId("u-1")));
        when(assignUseCase.assign("u-target", List.of("PLAYER", "OPERATOR"),
                "u-1", "PLATFORM_ADMIN", BUILDING_ID)).thenReturn(adminRequestDto());

        mvc.perform(post("/api/admin/users/u-target/roles")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[\"PLAYER\",\"OPERATOR\"]"))
                .andExpect(status().isAccepted())
                .andExpect(jsonPath("$.eventType").value("ROLE_ASSIGNMENT_REQUESTED"))
                .andExpect(jsonPath("$.status").value("PENDING"));

        verify(assignUseCase).assign("u-target", List.of("PLAYER", "OPERATOR"),
                "u-1", "PLATFORM_ADMIN", BUILDING_ID);
    }

    @Test
    void assignRoles_401_whenCurrentUserIdEmpty() throws Exception {
        ...
        verifyNoInteractions(assignUseCase);
    }
}