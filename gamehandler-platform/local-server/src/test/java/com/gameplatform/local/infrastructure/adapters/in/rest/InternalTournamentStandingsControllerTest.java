...
/**
 * Slice test for {@link InternalTournamentStandingsController}. Mirrors the
 ...
 * <p>Coverage:
 * <ul>
 *   <li>{@code PUT /internal/tournaments/standings/sync} 200 on a valid body with correct {@code X-Internal-Api-Key};</li>
...
class InternalTournamentStandingsControllerTest {

    private static final String INTERNAL_API_KEY = "test-internal-key-123";

    private final ObjectMapper objectMapper = new ObjectMapper().registerModule(new JavaTimeModule());

    @Mock TournamentStandingsLocalSyncService tournamentStandingsLocalSyncService;
    private MockMvc mvc;
    ...
    private static TournamentStandingsEventDto sampleDto(String tournamentId) {
        return new TournamentStandingsEventDto(
                "evt-" + tournamentId,
    ...
    @Test
    void syncTournamentStandings_200_whenApiKeyValidAndBodyIsValid() throws Exception {
        TournamentStandingsEventDto dto = sampleDto("t-1");

        mvc.perform(put("/internal/tournaments/standings/sync")
                        .header("X-Internal-Api-Key", INTERNAL_API_KEY)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(List.of(dto))))
                .andExpect(status().isOk());

        verify(tournamentStandingsLocalSyncService).applyEvents(any());
    }

    @Test
    void syncTournamentStandings_200_onEmptyBody() throws Exception {
        mvc.perform(put("/internal/tournaments/standings/sync")
                        .header("X-Internal-Api-Key", INTERNAL_API_KEY)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[]"))
    ...
    @Test
    void syncTournamentStandings_401_whenApiKeyMissing() throws Exception {
        TournamentStandingsEventDto dto = sampleDto("t-1");
        ...
        verifyNoInteractions(tournamentStandingsLocalSyncService);
    }

    @Test
    void syncTournamentStandings_401_whenApiKeyWrong() throws Exception {
        TournamentStandingsEventDto dto = sampleDto("t-1");
    ...
    @Test
    void syncTournamentStandings_isIdempotentAtHttpLayer_sameBodyAcceptedTwice() throws Exception {
        TournamentStandingsEventDto dto = sampleDto("t-1");
        ...
        verify(tournamentStandingsLocalSyncService, times(2)).applyEvents(any());
    }
}