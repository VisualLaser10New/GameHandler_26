...
public class LocalTournamentMatchRestAdapter implements PushTournamentMatchToLocalServersPort {
    ...
    private static final Logger log = LoggerFactory.getLogger(LocalTournamentMatchRestAdapter.class);
    ...
    public void pushTournamentMatch(List<TournamentMatchScheduledDto> events, RegisteredLocalServer server) {
        ...
        log.info("Pushing {} tournament-match events to local server at {}", events.size(), url);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("X-Internal-Api-Key", apiKey);

        HttpEntity<List<TournamentMatchScheduledDto>> entity = new HttpEntity<>(events, headers);
        ...
        try {
            retryTemplate.execute(new org.springframework.retry.RetryCallback<Void, Exception>() {
                ...
                public Void doWithRetry(RetryContext context) throws Exception {
                    try {
                        ...
                        log.info("Successfully pushed tournament-match events to local server at {}", url);
                        return null;
                    } catch (Exception e) {
                        if (isTransient(e)) {
                            log.warn("Transient failure pushing tournament-matches to local server at {} (attempt {}). Retrying...",
                                    url, context.getRetryCount() + 1, e);
                            throw new TransientPushException("Transient failure pushing tournament-matches to " + url, e);
                        } else {
                            log.error("Non-transient failure pushing tournament-matches to local server at {}", url, e);
                            throw e;
                        }
                    }
                }
            ...
            if (isConnectionRefusedRoot(e)) {
                ...
                log.error("Failed to push tournament-matches to local server at {} after retries.", url, e);
            }
            throw new RuntimeException("Failed to push tournament-matches to local server: " + url, e);
        }
    }
