...
/**
 * Adapter REST che invia eventi {@code TEAM_MEMBERS_UPSERTED} al local server
 * tramite l'endpoint
 * {@code PUT /internal/tournaments/teams/members/sync}.
 *
 * <p>Gemello strutturale di {@link LocalTournamentParticipantsRestAdapter}
 * (stesso wiring SSLContext, timeout, header {@code X-Internal-Api-Key} e
 * {@link RetryTemplate} a 3 tentativi). Il corpo della richiesta &egrave;
 * {@code List<TeamMembersEventDto>}; l'upsert lato ricevente &egrave;
 * un'operazione delete+insert idempotente per {@code tournamentId}, quindi un
 * errore di trasporto transiente viene ritentato tramite outbox al successivo
 * ciclo dello scheduler.</p>
 *
 * @see LocalTournamentParticipantsRestAdapter
 * @see PushTeamMembersToLocalServersPort
 */
@Component
public class LocalTeamMembersRestAdapter implements PushTeamMembersToLocalServersPort {

    private static final Logger log = LoggerFactory.getLogger(LocalTeamMembersRestAdapter.class);

    private static final String ENDPOINT_PATH = "/internal/tournaments/teams/members/sync";
    ...
    public LocalTeamMembersRestAdapter(
     ...
     * @param apiKey       chiave API interna inviata nell'header {@code X-Internal-Api-Key}; non deve essere {@code null}
     */
    LocalTeamMembersRestAdapter(RestTemplate restTemplate, String apiKey) {
    ...
    /**
     * Invia una lista di eventi di membri del team al local server specificato
     ...
     * originale.
     *
     * @param events la lista degli eventi di membri del team da inviare; non deve essere {@code null}
     ...
     */
    @Override
    public void push(List<TeamMembersEventDto> events, RegisteredLocalServer server) {
        String url = server.getBaseUrl() + ENDPOINT_PATH;
        log.info("Pushing {} team-members events to local server at {}", events.size(), url);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("X-Internal-Api-Key", apiKey);

        HttpEntity<List<TeamMembersEventDto>> entity = new HttpEntity<>(events, headers);
        ...
        try {
            retryTemplate.execute(new org.springframework.retry.RetryCallback<Void, Exception>() {
                ...
                public Void doWithRetry(RetryContext context) throws Exception {
                    try {
                        ...
                        log.info("Successfully pushed team-members events to local server at {}", url);
                        return null;
                    } catch (Exception e) {
                        if (isTransient(e)) {
                            log.warn("Transient failure pushing team-members to local server at {} (attempt {}). Retrying...",
                                    url, context.getRetryCount() + 1, e);
                            throw new TransientPushException("Transient failure pushing team-members to " + url, e);
                        } else {
                            log.error("Non-transient failure pushing team-members to local server at {}", url, e);
                            throw e;
                        }
                    }
                }
            ...
            if (isConnectionRefusedRoot(e)) {
                ...
                log.error("Failed to push team-members to local server at {} after retries.", url, e);
            }
            throw new RuntimeException("Failed to push team-members to local server: " + url, e);
        }
    }
